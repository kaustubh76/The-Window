...
- **Collateral custody uses a registered vault-operator EOA.** A Solidity contract cannot hold a BabyJubJub key or generate client-side proofs, so it cannot be a first-class eERC account. Custody of encrypted collateral sits in a **registered vault-operator EOA**; the `CollateralVault` contract holds the *authority* and records, and movement is event-driven. The **lock** is a two-step gate: a borrower calls `lockCollateral` with a valid solvency proof (state → `Requested`, emits `LockRequested`), the operator escrows the encrypted collateral and calls `confirmLock` (state → `Locked`) — and only a `Locked` loan can be funded (`LoanBook.confirmFunding` checks `isLocked`). The **terminal** moves (`release` on repay, `seizeTo` on default) finalize the vault state **atomically on-chain** in the same tx and emit `ReleaseOrdered`/`SeizeOrdered`; the operator executes the corresponding encrypted-collateral transfer off that event. There is deliberately no on-chain "confirm" for release/seize — the loan is already terminal, so there is nothing left to gate; an extra confirm would be theater. Authority is on-chain; custody is in the EOA.
