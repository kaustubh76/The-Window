...
- **Permissioned Avalanche L1 — BOTH primitives, composed (not mirrored).** eERC hides the **amount**; the L1 hides **participation** — the half eERC can't close on a public chain, and for a *stigma* market participation IS the signal. A sovereign L1 (`thewindowl1`, Subnet-EVM via `avalanche-cli`) where **market membership IS chain access**: one `MemberRegistry` drives four layers — market (`onlyMember`), eERC enrollment, network (`TxAllowList` write-gate synced by a keeper: non-members can't **transact**), and observation (`READ_GATE`: the market's read surface is member-signature-gated, so non-members can't **observe** it). One `removeMember` revokes all four **atomically** — a complete eviction impossible on shared public Fuji. Proven end-to-end: `verify_l1_allowlist` (write), `verify_l1_readgate` (read), `verify_l1_revoke` (atomic), `what_a_competitor_sees` (`demo/run_l1.sh`).
