...
This README is the single source of truth for the build. It is written to be executed by an AI coding agent (Claude Code) working with a human operator ("K"). Read §1 before writing any code.
...
| Benchmark Administrator | Holds the eERC **auditor** key; decrypts per-tick aggregates; computes clearing + matching; posts M-ONIA with proof of correct decryption. Accountable, rotatable role (eERC supports auditor key rotation `[TO-VERIFY]`) | Auditor keypair |
...
6. **Fund.** Lender executes an eERC encrypted transfer of principal to borrower, referencing the loanId. LoanBook marks the loan active on transfer confirmation `[TO-VERIFY: how third-party contracts observe eERC transfers — likely via events + reference data; derive the exact hook from the repo]`.
...
**Visible to the Administrator:** everything, by decryption under the auditor key. Mitigations: the role is (a) contractually bound to publish only aggregates, (b) auditable — every print carries a PoCD, (c) rotatable via eERC auditor key rotation `[TO-VERIFY]`. This is a *trusted, accountable* role, analogous to a benchmark administrator (cf. ICE Benchmark Administration) — never claim otherwise.
...
## 5. Architecture (mirrors `the_window_architecture.excalidraw`)
...
Install and pin: Foundry (latest), Node 20+, `circom` 2.x, `snarkjs`, `avalanche-cli` (for D7 stretch). Configure Fuji: RPC `https://api.avax-test.network/ext/bc/C/rpc`, chainId 43113, faucet AVAX for 4+ test EOAs (lender ×2, borrower, admin, keeper). Create `.env.example` with every variable any service reads. `[TO-VERIFY]` the eERC SDK package name and version on npm before adding it to package.json — read the official repo's own examples for the canonical import.
...
All contracts: Solidity ^0.8.24, Foundry, custom errors, events for every state transition, `onlyMember` where noted. No upgradeability, no proxies (hackathon).
...
### 9.2 AuctionHouse
State per epoch: status (Open/Closed/Printed), per-tick per-side accumulator `Ciphertext side x tick → C_agg`, bid records `(bidder, side, tick, C_size)` (ciphertext only). Functions: `submitAsk(tick, C_size, fundsProofRef)`, `submitBid(tick, C_size)`, `closeEpoch()` (keeper, after window), `getAggregates(epoch)`. Funds proof for lenders `[TO-VERIFY]`: if eERC exposes a balance-≥-x proof pattern, use it; if not, lenders pre-escrow encrypted funds into AuctionHouse via eERC transfer and unfilled amounts are returned post-match (simpler; prefer this if teardown says proofs-against-balances are nontrivial). One bid per member per tick per epoch (bound accumulator gas).
...
### 9.4 CollateralVault
`lockCollateral(loanId, C_collateral, solvencyProof)` — verifies Circuit 1 proof binding `(C_collateral, C_loanSize, h)`; escrows via eERC transfer from borrower to vault `[TO-VERIFY: transfer-to-contract semantics — the vault needs its own registered BJJ key; confirm contracts can hold eERC balances, else use a vault-controlled EOA pattern]`. `release(loanId)` (LoanBook-only), `seizeTo(loanId, lender)` (LoanBook-only).
...
| PoCD circuit blowup on array version | D4 midday | Split bid/ask proofs; if still blocked, print with single-sum PoCD per side + attested per-tick breakdown |
