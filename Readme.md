...
**Bonus track — permissioned L1 (implemented):** the full stack also runs on a sovereign Avalanche L1 (`thewindowl1`, chainId 43117, Subnet-EVM) where **MemberRegistry membership IS chain access**. This is *composition, not a mirror*: **eERC hides the amount; the L1 hides participation.** A keeper syncs the `TxAllowList` precompile from membership events so non-members cannot **transact**, and the market's read surface is member-gated (`READ_GATE`) so non-members cannot **observe** it either — closing the participation leak eERC can't close on a public chain (the stigma signal *is* participation, §4). One `MemberRegistry` drives four layers (market · eERC enrollment · network · observation), and one `removeMember` revokes all four atomically. Reproduce with `avalanche-cli` + [`demo/run_l1.sh`](demo/run_l1.sh) (proofs: `verify_l1_allowlist` / `verify_l1_readgate` / `verify_l1_revoke`); design + verified run in [`notes/09-permissioned-l1.md`](notes/09-permissioned-l1.md).
